...
int main( int argc, char **argv ) {              
	...
	char *line_checker = NULL;
    size_t length = 0;

    if (getline(&line_checker, &length, fp) == -1 ) {
         printf("Error reading the input file.\n");
         free(line_checker);
         exit(1);
     }
    char *first_line = NULL;
    first_line = strtok(line_checker, DELIM); 
	...
	// Since we used getline on the first line already, calling it again will read
	// the second line. So my solution to this is to close the file and reopen it again.
